...
SPOT = pd.read_csv(os.path.join(DIR, 'data/price_demand_nsw.csv'),
                   index_col='datetime', parse_dates=True).iloc[:, 0]

# SEASONAL = pd.read_csv(os.path.join(DIR, 'seasonal_new.csv'),
...
HOLIDAY = Holiday(os.path.join(DIR, 'data/holiday_nsw.txt'))
