...
def get_args():
    ...
    parser.add_argument('--lmd', default=0.002, type=float,)
    parser.add_argument('--alpha', default=0.05, type=float,)
    parser.add_argument('--s', default=50, type=int,)
    parser.add_argument('--seasonal', default=0, type=bool,)
    parser.add_argument('--flag', default='rnn-lube', type=str,)
...
if __name__ == '__main__':
    ...
    print(testX.shape)
    print(testY.shape)
    train_period_output = trainY.data.numpy().reshape(-1) * TOTAL_STD
    test_period_output = testY[-TEST_LENGTH:].data.numpy().reshape(-1) * TOTAL_STD
    with torch.no_grad():
    ...
    for epoch in tqdm(range(epochs)):
        ...
        train_lower_bound = train_period_forecast[:, :24].data.cpu().numpy().reshape(-1)
        train_upper_bound = train_period_forecast[:, 24:].data.cpu().numpy().reshape(-1)
        ...
        test_lower_bound = test_period_forecast[:, :24].data.cpu().numpy().reshape(-1)
        test_upper_bound = test_period_forecast[:, 24:].data.cpu().numpy().reshape(-1)
