...
class NN(nn.Module):
    def __init__(self, input_dim, hidden, output_dim, bn=0, dropout=1):
        super(NN, self).__init__()
        self.layers = nn.Sequential()
        L = len(hidden)
        layer_in = input_dim
        i = 0
        for layer_out in hidden:
            self.layers.add_module(module=nn.Linear(layer_in, layer_out), name='Linear'+str(i))
            if bn:
                self.layers.add_module(module=nn.BatchNorm1d(layer_out), name='BN'+str(i))
            self.layers.add_module(module=nn.Sigmoid(), name='Act'+str(i))
            self.layers.add_module(module=nn.Dropout(dropout), name='Dropout'+str(i))
            layer_in = layer_out
            i += 1
        self.h2o = nn.Linear(layer_in, output_dim)
