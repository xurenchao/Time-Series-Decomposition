...
class multi_dRNN_with_dilations(nn.Module):
    ...
    """ 
    def __init__(self, input_dims, hidden_structs, dilations):
        ...
        self.h2o = nn.Linear(lastHiddenDim, input_dims)
    ...
    def forward(self, x):
        ...
        for input_t in x.split(1, dim=1):
            htop = self.multi_dRNN(input_t.squeeze(1))
            output = F.tanh(self.h2o(htop))
            outputs += [output]
    ...
    def dRNN(self, cell, inputs, rate):
        ...
        print(n_steps, batch_size, hidden_size, rate)
        ...
        if not EVEN:
            ...
            zero_tensor = inputs[0].data.new(inputs[0].data.size()).zero_()
        ...
        print(dialated_n_steps)
        dilated_inputs = [torch.cat([inputs[i * rate + j] for j in range(rate)], dim=1) for i in range(dialated_n_steps)]
        print(dilated_inputs.shape)
        dilated_outputs = []
        hidden, cstate = self.init_hidden(batch_size*rate, hidden_size)
        for dilated_input in dilated_inputs:
            print(dilated_input.shape, hidden.shape)
    ...
    def init_hidden(self, batch_size, hidden_dim):
        ...
        return (torch.zeros(batch_size, hidden_dim), torch.zeros(batch_size, hidden_dim))
