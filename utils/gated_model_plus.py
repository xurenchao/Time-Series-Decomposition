...
class ResRNN(nn.Module):
    ...
    def forward(self, x):
        hidden1 = hidden2 = hidden3 = self.init_hidden_state()
        b_h1 = b_h2 = b_h3 = b_z1 = b_z2 = self.init_bias(self.hidden_size)
        b_o1 = b_o2 = self.init_bias(self.output_dim)
        z1 = z2 = self.init_gate_state()
        outputs = []
        for input_t in x.split(1, dim=1):
            hidden1 = F.tanh(self.l1_W(input_t.squeeze(1)) + self.l1_U(hidden1) + b_h1)
            hidden2 = z1 * F.tanh(self.l2_W(hidden1) + self.l2_U(hidden2) + b_h2) + (1 - z1) * hidden2
            hidden3 = z2 * F.tanh(self.l3_W(hidden2) + self.l3_U(hidden3) + self.l3_V(hidden1) + b_h3) + (1 - z2) * hidden3

            z1 = F.sigmoid(self.gate1(torch.cat((hidden1, hidden2), 1)) + b_z1).squeeze(0)
            z2 = F.sigmoid(self.gate2(torch.cat((hidden1, hidden2, hidden3), 1)) + b_z2).squeeze(0)
            if self.hard_gate:
                z1 = self.binary(z1)
                z2 = self.binary(z2)
            z1 = z1 * z2

            output = F.tanh(self.out1(hidden2) + b_o1) + F.tanh(self.out2(hidden3) + b_o2)
            outputs += [output]
    ...
    def forecast(self, x):
        ...
        for input_t in x.split(1, dim=1):
            ...
            output1 = F.tanh(self.out1(hidden2) + b_o1)
            output2 = F.tanh(self.out2(hidden3) + b_o2)
            output = output1 + output2
